exports.ox_target:addModel('p_ld_stinger_s', {
    {
        ...
        label = 'Pegar espinhos',
        onSelect = function(data)
            if lib.progressBar({
                duration = 1500,
                label = 'Pegando espinhos',
                useWhileDead = false,
                canCancel = true,
                disable = {
                    car = true,
...
exports('deploySpikestrip', function()
    if cache.vehicle then
        lib.notify({title = 'Não pode implantar um Spikestrip enquanto em um veículo', type = 'error'})
        return
    end
    ...
    elseif count > 1 then
        ...
        size = lib.inputDialog('Implantar Spikestrip', {
