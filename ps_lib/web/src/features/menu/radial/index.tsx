...
const RadialMenu: React.FC = () => {
	...
	useNuiEvent(
		...
		async (
			data:
				| { items: RadialMenuItem[]; sub?: boolean; option?: string }
				| false
		) => {
				...
				<ScaleFade visible={visible}>
					<svg width="350px" height="350px" transform="rotate(90)">
						...
						{menuItems.map((item, index) => {
							...
							const pieAngle =
								360 /
								(menuItems.length < 3 ? 3 : menuItems.length);
